...
from src.handler.schemas import PlayerQueueEntry
from src.storage.models import Match, MatchStatus, User
from src.storage.repositories.match_repository import (
    get_match_by_id,
    finish_match_with_winner,
)
from src.storage.repositories.user_repository import get_users_by_ids
...
async def select_problem_for_match(
    ...
    """
    from src.storage.repositories.match_repository import (
        select_problem_for_match as select_problem_db,
    )

    try:
        # Call the repository function to select a problem
        problem_id = await select_problem_db(
            db, player1_id, player2_id, player1_rating, player2_rating
        )
        ...
        match_logger.info(
            f"Selected problem {problem_id} for match between players {player1_id} and {player2_id}"
        )
        return problem_id
...
async def capitulate_match_logic(
    ...
    from src.service.services.match_rating import RatingService
    from src.handler.routes.submission import submission_logger
    ...
    if not winner or not loser:
        raise ResourceNotFoundException("Could not load player storage")
