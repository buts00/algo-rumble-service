...
def calculate_new_rating(current_rating: int, expected_score: float, actual_score: float) -> int:
    """
...
async def update_ratings_after_match(
    db: AsyncSession, 
    winner_id: Union[UUID, str], 
    loser_id: Union[UUID, str]
) -> Tuple[int, int]:
    """
    ...
    if not winner or not loser:
        logger.error(f"Failed to update ratings: User not found (winner_id={winner_id}, loser_id={loser_id})")
        return None, None
...
async def update_ratings_for_draw(
    db: AsyncSession, 
    player1_id: Union[UUID, str], 
    player2_id: Union[UUID, str]
) -> Tuple[int, int]:
    """
    ...
    if not player1 or not player2:
        logger.error(f"Failed to update ratings: User not found (player1_id={player1_id}, player2_id={player2_id})")
        return None, None
