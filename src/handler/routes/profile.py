...
from src.service.services.auth_dependency import get_current_user
from src.service.services.profile import (
    get_user_match_history_service,
    get_user_contribution_calendar_service,
    get_user_rating_history_service,
    get_user_topic_stats_service,
)
from src.config import logger
from src.storage.repositories import get_session
...
async def get_user_contribution_calendar(
    ...
    """
    profile_logger.info(
        f"Contribution calendar request for user ID: {user_id}, year: {year}"
    )
    ...
    try:
        ...
        profile_logger.error(
            f"Unexpected error during contribution calendar request: {str(e)}"
        )
        raise DatabaseException(detail="An unexpected error occurred")
...
async def get_user_rating_history(
    ...
    try:
        ...
        # Get rating history
        rating_history = await get_user_rating_history_service(db, user_uuid)
        profile_logger.info(
        ...
        profile_logger.error(
            f"Unexpected error during rating history request: {str(e)}"
        )
        raise DatabaseException(detail="An unexpected error occurred")
...
async def get_user_topic_stats(
    ...
    This storage can be used to create a polygon visualization of the user's strengths.
    ...
    try:
        ...
        topic_stats = await get_user_topic_stats_service(db, user_uuid, limit)
        profile_logger.info(
        ...
        profile_logger.error(
            f"Unexpected error during topic statistics request: {str(e)}"
        )
        raise DatabaseException(detail="An unexpected error occurred")
