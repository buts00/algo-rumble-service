...
from src.handler.schemas import UserResponseModel, UserCreateModel, UserLoginModel
...
from src.storage.repositories import RedisClient, get_redis_client, get_session
...
@auth_router.get(
    ...
    description="Returns the storage of the currently authenticated user based on the access token.",
)
async def get_current_user(
    ...
    auth_logger.debug(f"Fetching storage for user ID: {user_id}")
    user = await user_service.get_user_by_id(user_id, session)
    if not user:
        auth_logger.warning(f"User not found: ID {user_id}")
