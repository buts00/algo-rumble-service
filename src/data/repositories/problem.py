...
from src.data.schemas import ProblemCreate, ProblemResponse, TestCaseCreate, TestCaseResponse
...
async def create_problem_in_db(db: AsyncSession, problem_data: ProblemCreate) -> ProblemResponse:
...
async def create_testcases_in_db(db: AsyncSession, problem_id: uuid.UUID, testcases: List[Dict[str, str]]) -> TestCaseResponse:
    ...
    try:
        ...
        for idx, tc in enumerate(testcases, 1):
            ...
            testcase_data_json = json.dumps({"input": tc["input"], "output": tc["output"]})
        ...
        problem_logger.info(f"Created {len(testcases)} testcases for problem ID: {problem_id}")
        return TestCaseResponse(problem_id=problem_id, testcase_ids=testcase_ids)
    except ResourceNotFoundException:
        raise
    except Exception as e:
        problem_logger.error(f"Failed to create testcases for problem {problem_id}: {str(e)}")
...
async def update_problem_in_db(db: AsyncSession, problem_id: uuid.UUID, update_data: Dict[str, Any]) -> ProblemResponse:
...
async def list_problems_from_db(db: AsyncSession, skip: int, limit: int) -> List[ProblemResponse]:
    """List problems with pagination."""
    try:
        result = await db.execute(
            select(Problem).offset(skip).limit(limit).order_by(Problem.created_at.desc())
        )
        problems = result.scalars().all()
        problem_logger.info(f"Listed {len(problems)} problems with skip: {skip}, limit: {limit}")
